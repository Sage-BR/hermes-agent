...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    print("  Re-run `hermes update` to retry; if the agent won't start, reinstall from https://github.com/Sage-BR/hermes-agent")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    _download_and_swap_zip(branch, f"https://github.com/Sage-BR/hermes-agent/archive/refs/heads/{branch}.zip")
