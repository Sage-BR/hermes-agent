...
class TestOpenRouterCatalogDiskCache:
    """A cold process serves the curated picker list from disk while it is fresh."""
    ...
    def _install_network(monkeypatch, calls):
        ...
        def fake_index(url, timeout, opener):
            calls.append(url)
            return payload, {item["id"]: item for item in payload}
    ...
    def test_fresh_snapshot_serves_cold_process_without_network(self, monkeypatch):
        calls = []
        self._install_network(monkeypatch, calls)

        first = fetch_openrouter_models()
        assert calls
        assert _models_mod._openrouter_catalog_disk_path().is_file()
    ...
    def test_expired_or_corrupt_snapshot_falls_through_to_fetch(self, monkeypatch):
        calls = []
        self._install_network(monkeypatch, calls)
